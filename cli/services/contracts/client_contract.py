...
class ClientContract(ContractService):
    _CLIENT_CONTRACT_ADDRESS: EthAddress | None = None

    def __init__(self, contract_address: EthAddress | None = None):
        if not contract_address and not ClientContract._CLIENT_CONTRACT_ADDRESS:
            from cli.services.contracts.porep_market import PoRepMarket
            ClientContract._CLIENT_CONTRACT_ADDRESS = PoRepMarket().get_client_smart_contract_address()

        # noinspection PyTypeChecker
        super().__init__(contract_address or ClientContract._CLIENT_CONTRACT_ADDRESS,
