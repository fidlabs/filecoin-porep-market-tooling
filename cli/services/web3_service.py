...
@singledispatch
def _to_eth(addr: object) -> EthAddress:
    raise ValueError(f"Cannot convert {type(addr).__name__} to EthAddress: {addr!r}")

@_to_eth.register(EthAddress)
def _(addr: EthAddress) -> EthAddress:
    return addr

@_to_eth.register(FilAddress)
def _(addr: FilAddress) -> EthAddress:
    return EthAddress.from_filecoin_address(str(addr))

@_to_eth.register(ActorId)
def _(addr: ActorId) -> EthAddress:
    return _to_eth(FilAddress(str(addr)))

@_to_eth.register(str)
def _(addr: str) -> EthAddress:
    if Web3.is_address(addr):
        return EthAddress(addr)
    actor = ActorId.try_parse(addr)
    if actor is not None:
        return _to_eth(actor)
    fil = FilAddress.try_parse(addr)
    if fil is not None:
        return _to_eth(fil)
    raise ValueError(f"Cannot parse as EthAddress: {addr!r}")


@singledispatch
def _to_fil(addr: object) -> FilAddress:
    raise ValueError(f"Cannot convert {type(addr).__name__} to FilAddress: {addr!r}")

@_to_fil.register(FilAddress)
def _(addr: FilAddress) -> FilAddress:
    return addr

@_to_fil.register(ActorId)
def _(addr: ActorId) -> FilAddress:
    return FilAddress(str(addr))

@_to_fil.register(EthAddress)
def _(addr: EthAddress) -> FilAddress:
    response = Web3Service().w3().provider.make_request(
        RPCEndpoint("Filecoin.EthAddressToFilecoinAddress"), [str(addr)]
    )
    if "error" in response:
        raise RuntimeError(response["error"])
    return FilAddress(response["result"])

@_to_fil.register(str)
def _(addr: str) -> FilAddress:
    fil = FilAddress.try_parse(addr)
    if fil is not None:
        return fil
    if Web3.is_address(addr):
        return _to_fil(EthAddress(addr))
    actor = ActorId.try_parse(addr)
    if actor is not None:
        return _to_fil(actor)
    raise ValueError(f"Cannot parse as FilAddress: {addr!r}")


@singledispatch
def _to_actor(addr: object) -> ActorId:
    raise ValueError(f"Cannot convert {type(addr).__name__} to ActorId: {addr!r}")

@_to_actor.register(ActorId)
def _(addr: ActorId) -> ActorId:
    return addr

@_to_actor.register(FilAddress)
def _(addr: FilAddress) -> ActorId:
    actor = ActorId.try_parse(str(addr))
    if actor is not None:
        return actor
    response = Web3Service().w3().provider.make_request(
        RPCEndpoint("Filecoin.StateLookupID"), [str(addr), None]
    )
    if "error" in response:
        raise RuntimeError(response["error"])
    if not response["result"]:
        raise RuntimeError(f"Empty result for StateLookupID({addr})")
    return ActorId(response["result"])

@_to_actor.register(EthAddress)
def _(addr: EthAddress) -> ActorId:
    return _to_actor(_to_fil(addr))

@_to_actor.register(str)
def _(addr: str) -> ActorId:
    actor = ActorId.try_parse(addr)
    if actor is not None:
        return actor
    fil = FilAddress.try_parse(addr)
    if fil is not None:
        return _to_actor(fil)
    if Web3.is_address(addr):
        return _to_actor(EthAddress(addr))
    raise ValueError(f"Cannot parse as ActorId: {addr!r}")

EthAddress.parse = staticmethod(_to_eth)
FilAddress.parse = staticmethod(_to_fil)
ActorId.parse    = staticmethod(_to_actor)
